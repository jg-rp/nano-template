...
static PyObject *undefined(ML_ObjPage *page, Py_ssize_t end_pos,
...
static PyObject *eval_not_expr(ML_Expr *expr, ML_Context *ctx)
{
    ...
    op = ML_Expression_evaluate(expr->right, ctx);
    if (!op)
    {
        goto cleanup;
    }
...
static PyObject *eval_and_expr(ML_Expr *expr, ML_Context *ctx)
{
    ...
    left = ML_Expression_evaluate(expr->left, ctx);
    if (!left)
    {
        goto cleanup;
    }
    ...
    right = ML_Expression_evaluate(expr->right, ctx);
    if (!right)
    {
        goto cleanup;
    }
...
static PyObject *eval_var_expr(ML_Expr *expr, ML_Context *ctx)
{
    ...
    if (ML_Context_get(ctx, expr->head->objs[0], &op) < 0)
    {
